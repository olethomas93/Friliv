  ...
  <q-card v-if="weatherData" class="cards text">
    <q-card-section>
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel
          class="tab"
          v-for="(forcast, index1) in weatherData"
          :key="forcast"
          :name="index1"
        >
          <div class="wrapper">
            <div class="iconWeather">
              <img
                v-if="forcast[index].data.next_1_hours.summary.symbol_code"
                :src="
                  'img/weather/' +
                  forcast[index].data.next_1_hours.summary.symbol_code +
                  '.svg'
                "
                style="width: 20%"
                alt="ds"
            ...
            <div>
              <div class="card-body" v-if="forcast">
                {{
                  roundNumber(forcast[0].data.instant.details.air_temperature)
                }}&#xb0;
              </div>
            </div>
          </div>
    ...
    <q-card-actions class="action">
      <q-tabs v-model="tab" class="text-teal">
        <q-tab
          v-for="(forcast, index) in weatherData"
          :key="forcast"
          :name="index"
          :label="convertTime(forcast[0].time)"
        />
      </q-tabs>
    </q-card-actions>
  </q-card>
