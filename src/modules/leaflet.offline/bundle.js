(function (global, factory) {
  ...
  typeof exports === "object" && typeof module !== "undefined"
    ? factory(exports, require("leaflet"), require("idb"))
    : typeof define === "function" && define.amd
    ? define(["exports", "leaflet", "idb"], factory)
    : ((global =
        typeof globalThis !== "undefined" ? globalThis : global || self),
      factory((global.LeafletOffline = {}), global.L, global.idb));
})(this, function (exports, leaflet, idb) {
  "use strict";
  ...
  async function downloadTile(tileUrl) {
    ...
    if (!response.ok) {
      throw new Error(
        "Request failed with status ".concat(response.statusText)
      );
    }
  ...
  function getStoredTilesAsJson(layer, tiles) {
    ...
    for (let i = 0; i < tiles.length; i += 1) {
      const topLeftPoint = new leaflet.Point(
        tiles[i].x * layer.getTileSize().x,
        tiles[i].y * layer.getTileSize().y
      );
      const bottomRightPoint = new leaflet.Point(
        topLeftPoint.x + layer.getTileSize().x,
        topLeftPoint.y + layer.getTileSize().y
      );
      const topLeftlatlng = leaflet.CRS.EPSG3857.pointToLatLng(
        topLeftPoint,
        tiles[i].z
      );
      const botRightlatlng = leaflet.CRS.EPSG3857.pointToLatLng(
        bottomRightPoint,
        tiles[i].z
      );
  ...
  class TileLayerOffline extends leaflet.TileLayer {
    createTile(coords, done) {
      ...
      leaflet.DomEvent.on(
        tile,
        "load",
        leaflet.Util.bind(this._tileOnLoad, this, done, tile)
      );
      leaflet.DomEvent.on(
        tile,
        "error",
        leaflet.Util.bind(this._tileOnError, this, done, tile)
      );
      ...
      for (let index = 0; index < tilePoints.length; index += 1) {
        ...
        tiles.push({
          key: getTileUrl(this._url, {
            ...data,
            s:
              (_this$options$subdoma = this.options.subdomains) === null ||
              _this$options$subdoma === void 0
                ? void 0
                : _this$options$subdoma[0],
          }),
    ...
    _calculateTiles() {
      ...
      if (this.options.saveWhatYouSee) {
        ...
        if (currentZoom < minZoom) {
          throw new Error(
            "It's not possible to save with zoom below level ".concat(
              minZoom,
              "."
            )
          );
