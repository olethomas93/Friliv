import {
  ...
  import type {Coords,DoneCallback,TileLayerOptions} from 'leaflet'
  import {
    getTileUrl,
    getBlobByKey,
    getTilePoints,
  ...
  export class TileLayerOffline extends TileLayer {
    ...
    async setDataUrl(coords: { x: number; y: number; z: number }): Promise<string> {
      const data = await getBlobByKey(this._getStorageKey(coords));
     ...
     */
    _getStorageKey(coords: { x: number; y: number; z: number }): string {
